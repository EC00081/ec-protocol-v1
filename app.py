...
if user['role'] == "Exec":
    ...
    # PAGE 3: LOGS
        ...
        except: st.write("No Data")        for row in records:
            # Fuzzy PIN match
            for k, v in row.items():
                if str(k).lower().strip() == 'pin' and str(v).strip() == target:
                    return row
...
with st.sidebar:
    st.markdown("### 🧭 NAVIGATION")
    # This variable 'nav_selection' is now global to the script
    nav_selection = st.radio("GO TO:", ["LIVE DASHBOARD", "SCHEDULER", "LOGS"])
...
if user['role'] == "Exec":
    ...
    st.markdown(f"""
        <div class="hero-header">
            <h2 style='margin:0;'>EC ENTERPRISE</h2>
            <div style='background:rgba(59,142,219,0.2); color:#3b8edb; padding:5px 15px; border-radius:20px; display:inline-block; margin-top:10px;'>
                {user['name'].upper()}
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    if nav_selection == "LIVE DASHBOARD":
        ...
        if active:
            earnings += ((time.time() - st.session_state.user_state['start_time']) / 3600) * user['rate']
            # Reset start time to now to prevent double counting on refresh, keep running total in earnings
            st.session_state.user_state['start_time'] = time.time() 
            st.session_state.user_state['earnings'] = earnings
