...
# --- REPORTING ENGINE (PDF OR TEXT FALLBACK) ---
...
if 'logged_in_user' not in st.session_state:
    ...
    st.markdown("<p style='text-align: center; color: #10b981; letter-spacing: 3px; font-weight:600;'>ENTERPRISE PROTOCOL v6.0.4-Live</p><br>", unsafe_allow_html=True)
    with st.container():
...
if nav == "FLIGHT RISK RADAR":
    ...
    with c2:
        if user['level'] in ["Admin", "Executive", "Manager", "Director"]:
            file_data = generate_compliance_report(user['dept'], user['name'])
            ext = "pdf" if PDF_ACTIVE else "txt"
            mime = "application/pdf" if PDF_ACTIVE else "text/plain"
            if file_data:
                st.download_button(label=f"📄 Export Audit ({ext.upper()})", data=file_data, file_name=f"Vicentus_Audit_{user['dept']}_{date.today()}.{ext}", mime=mime)
